...
export async function main(ns) {
  ...
  const colorByClassName = (sel, col) =>
    doc.querySelectorAll(sel).forEach(el => (el.style.color = col));
  ...
  while (true) {
    try {
      ...
      hook1.insertAdjacentHTML(
        'beforeend',
        `<element class="HUD_GN_C HUD_el">${playerCity + '<br class="HUD_el">'}</element>`
      );
      colorByClassName('.HUD_GN_C', theme['cha']);
      ...
      hook1.insertAdjacentHTML(
        'beforeend',
        `<element class="HUD_GN_L HUD_el">${playerLocation + '<br class="HUD_el">'}</element>`
      );
      colorByClassName('.HUD_GN_L', theme['cha']);
      ...
      if (gangInfo != null) {
        ...
        hook1.insertAdjacentHTML(
          'beforeend',
          `<element class="HUD_GN_F HUD_el">${gangFaction + '<br class="HUD_el">'}</element>`
        );
        colorByClassName('.HUD_GN_F', theme['int']);
        ...
        hook1.insertAdjacentHTML(
          'beforeend',
          `<element class="HUD_GN HUD_el">${'$' + gangIncome + '/sec<br class="HUD_el">'}</element>`
        );
      ...
      if (corpService.hasCorp()) {
        ...
        hook1.insertAdjacentHTML(
          'beforeend',
          `<element class="HUD_Corp_D HUD_el">${
            '$' + dividendEarnings + '/sec<br class="HUD_el">'
          }</element>`
        );
        colorByClassName('.HUD_Corp_D', theme['rep']);
      }
      ...
      hook0.insertAdjacentHTML(
        'beforeend',
        `<element class="HUD_ScrInc_H HUD_el" title="Money Gain from Scripts per Second.">Script Income &nbsp;&nbsp;&nbsp;</element>`
      );
      colorByClassName('.HUD_ScrInc_H', theme['money']);
      hook1.insertAdjacentHTML(
        'beforeend',
        `<element class="HUD_ScrInc HUD_el">${'$' + scriptIncome + '/sec'}</element>`
      );
      ...
      hook0.insertAdjacentHTML(
        'beforeend',
        `<element class="HUD_ScrExp_H HUD_el" title="XP Gain from Scripts per Second."><br>Script Exp &nbsp;&nbsp;&nbsp;</element>`
      );
      colorByClassName('.HUD_ScrExp_H', theme['hack']);
      hook1.insertAdjacentHTML(
        'beforeend',
        `<element class="HUD_ScrExp HUD_el"><br>${scriptXP + ' XP/sec'}</element>`
      );
      ...
      hook1.insertAdjacentHTML(
        'beforeend',
        `<element class="HUD_Karma HUD_el"><br>${Math.floor(playerKarma)}</element>`
      );
